...
class LayerNormalization(tf.keras.layers.Layer):
    ...
    def call(self, inputs, axis=-1, epsilon=1e-5):
        mean, variance = tf.compat.v1.nn.moments(inputs, axis, keep_dims=True)
...
class Block(tf.keras.layers.Layer):
    ...
    def call(self, inputs, cache=None, dropout=None, attention_dropout=None,
        ...
        if return_cache:
            a, cache = a
        x = x + a
        # This part could go in any order
        m = self.mlp(inputs=x,
                     dropout=dropout)
        x = x + m
        if return_cache:
...
class Transformer(tf.keras.Model):
    ...
    def call(self, inputs, cache=None, dropout=None, attention_dropout=None,
             return_cache=False, blocks=None, use_2d=False, shape=None, comm=None, rank=None, size=None):
        """
...
class Embedding(tf.keras.layers.Layer):
    ...
    def call(self, inputs, start=None):
        ...
        x = tf.gather(self.word_embedding,
                      inputs)  # tf.gather: Gather slices from params axis axis according to indices.
        if start is None:
            start = 0
        end = start + shape[1]
...
class GPT2(tf.keras.Model):
    ...
    def call(self, inputs, cache=None,
             ...
             return_cache=False, return_logits=True, use_2d=False, parallelize=True):
        """
        ...
        """
        print("Hi, from line 530 of /src/gpt2_keras/builder/gpt2.py")
        print("****** from builer, parallelize= ", parallelize)
        if cache is not None:
            _cache = cache[0]["key"]
